...
class Ball:
    ...
    def move(self):
        ...
        if self.y <= self.r:
            ...
            self.vx *= 0.5
    def draw(self):
        """рисует элемент класса (отрисовывает шары)"""
        pygame.draw.circle(
            self.screen,
            self.color,
            (self.x, self.y),
...
class Gun:
    ...
    def drive(self, direction):
        """хотим двигать"""
        if direction == 'up' and self.y>100:
            self.y-=self.vy
...
class Target:
    ...
    def new_target(self):
        ...
        self.x = randint(200, 780)
        self.y = randint(0, 550)
        m = [10,15,30,40,100] #все возможные радиусы
        self.r = m[randint(0,len(m)-1)]
        self.vx = 1/self.r*randint(1,25)
        self.vy = 1/self.r*randint(1,25)
    ...
    def move(self):
        ...
        if self.x >= WIDTH - 200 - self.r or self.x <= self.r+200:
            if self.x > WIDTH -200 - self.r:
                self.x = WIDTH - 200 - self.r
            if self.x  <= self.r:
                self.x = self.r
            self.vx *= -1
...
bullet = 0
balls = []
#привязываем к времени
clock = pygame.time.Clock()
gun = Gun(screen,x=40,side=1)
gun2 = Gun(screen,x=1100,side=-1)
trg = [Target(screen) for i in range(0,100)] #создание элементов
...
while not finished:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 3:
         ...
         #  gun2.fire2_start(event)   #обнаруж координаты клавиш
         #  print(event.key)
